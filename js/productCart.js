...
function setCartIcon(obj, iconPath) {
    var oldImgUrl = obj.attr("src"),
        token = oldImgUrl.indexOf("images/") >= 0 ? "images/" : "image/",
        newUrl = oldImgUrl;
...
function animateCart(articleId) {
    setCartIcon($('#cartimg_' + articleId), "a-c/cart_animation.gif")
	doWait=true;
}
...
function showProductInCartStatus(articleId){
	if (doWait && clicked){
        ...
        if (ele && ele.value > 0){
            setCartIcon($('#cartimg_' + articleId), "a-c/cart_success.gif");
            clicked = false;
        }
    }
}
